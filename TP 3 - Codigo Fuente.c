...
void ADC0_IRQHandler(){	// (no se le puede cambiar el nombre)
	...
	if(Conversion == Sensor_luz){	// Conversion : Sensor_luz
		// Nunca supera el rango superior
		// Leo el valor (esto también limpia la flag de interrupción del ADC)
		if (ADC0->R[0] < MAX_LUZ_ADC){	// Supera el rango inferior
			TPM0->CONTROLS[2].CnV = MAX_VAL;	// Se traduce a la máxima iluminación
		}
		else TPM0->CONTROLS[2].CnV = K_LUZ*(MAX_VAL - ADC0->R[0]);	// Led verde
